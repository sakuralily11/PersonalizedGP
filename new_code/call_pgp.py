...
class personalizedGP(gpflow.models.GPR):
    def __init__(self, X_tr, Y_tr, kernel):
        super(self, personalizedGP).__init__(X_tr, Y_tr, kern=kernel)

        self.X_tr = X_tr 
        self.Y_tr = Y_tr

        # Build sGP model 
        sourceGP = gpflow.models.GPR(X_tr, Y_tr, kern=kernel)
        sourceGP.likelihood.variance = np.exp(2*np.log(np.sqrt(0.1*np.var(Y_tr))))
        max_x = np.amax(X_tr, axis=0)
        min_x = np.amin(X_tr, axis=0)
        sourceGP.kern.lengthscales = np.array(np.median(max_x - min_x))
        sourceGP.kern.variance = np.var(Y_tr)

        # Optimize model 
        sourceGP.compile()
        opt = gpflow.train.ScipyOptimizer()
        opt.minimize(sourceGP)

        # Store parameters 
        self.ls = sourceGP.read_values()['GPR/kern/lengthscales']
        self.var = sourceGP.read_values()['GPR/kern/variance']
        self.lik = float(sourceGP.likelihood.variance.value)
    ...
    def predict(X_ad, Y_ad, X_te):
        """ Trains and predicts on personalizedGP """ 

        m_adapt, s_adapt = None, None
        
        K_ts_star_all = self.__K(self.X_tr, X_te)
        K_tt_all = self.__K(X_ad)
        K_t_star_all = self.__K(X_ad, X_te)
        
        K_s = self.__K(self.X_tr)
        L_arg = K_s + self.lik*np.identity(K_s.shape[0]) 
        L = jitChol(L_arg) 
        
        alpha_denom = np.linalg.lstsq(L, self.Y_tr)[0]
        alpha = np.linalg.lstsq(L.transpose(),alpha_denom)[0]

        for i in range(1, len(x_a)+1): # 1:20 (or 1:21 exclusive)

            if i == 1:
                m_adapt = np.append(m_adapt, [m_s[0]]) 
                s_adapt = np.append(s_adapt, [s_s[0]]) 
...
class targetGP(gpflow.models.Model):
